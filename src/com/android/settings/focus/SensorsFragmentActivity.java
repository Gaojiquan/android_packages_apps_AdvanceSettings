...
public class SensorsFragmentActivity extends PreferenceFragment {
	...
	private static final boolean sHasTouchkeyToggle = DeviceSetUtils
			.fileExists(FILE_TOUCHKEY_TOGGLE);
	private static final boolean sHasTouchkeyBLN = DeviceSetUtils
			.fileExists(FILE_BLN_TOGGLE);
	...
	public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen,
		...
		if (key.compareTo(DeviceSettings.KEY_USE_GYRO_CALIBRATION) == 0) {
			boxValue = (((CheckBoxPreference) preference).isChecked() ? "1"
					: "0");
			DeviceSetUtils.writeValue(FILE_USE_GYRO_CALIB, boxValue);
			...
			DeviceSetUtils.writeValue(FILE_USE_GYRO_CALIB, "0");
			DeviceSetUtils.writeValue(FILE_USE_GYRO_CALIB, "1");
			DeviceSetUtils.showDialog((Context) getActivity(), "Calibration done",
					"The gyroscope has been successfully calibrated!");
		} else if (key.compareTo(DeviceSettings.KEY_TOUCHKEY_LIGHT) == 0) {
			DeviceSetUtils.writeValue(FILE_TOUCHKEY_LIGHT,
					((CheckBoxPreference) preference).isChecked() ? "0" : "1");
			DeviceSetUtils.writeValue(FILE_TOUCHKEY_TOGGLE,
					((CheckBoxPreference) preference).isChecked() ? "255" : "0");
		} else if (key.compareTo(DeviceSettings.KEY_TOUCHKEY_BLN) == 0) {
			DeviceSetUtils.writeValue(FILE_BLN_TOGGLE,
	...
	public static void restore(Context context) {
		...
		// When use gyro calibration value is set to 1, calibration is done at
		// the same time, which
		...
		if (!sharedPrefs.getBoolean(
				DeviceSettings.KEY_USE_GYRO_CALIBRATION, true))
			DeviceSetUtils.writeValue(FILE_USE_GYRO_CALIB, "0");

		if (sHasTouchkeyToggle) {
			DeviceSetUtils.writeValue(FILE_TOUCHKEY_LIGHT, sharedPrefs.getBoolean(
					DeviceSettings.KEY_TOUCHKEY_LIGHT, true) ? "0" : "1");
			DeviceSetUtils.writeValue(FILE_TOUCHKEY_TOGGLE, sharedPrefs.getBoolean(
					DeviceSettings.KEY_TOUCHKEY_LIGHT, true) ? "1" : "2");
		}

		if (sHasTouchkeyBLN) {
			DeviceSetUtils.writeValue(FILE_BLN_TOGGLE, sharedPrefs.getBoolean(
					DeviceSettings.KEY_TOUCHKEY_BLN, true) ? "1" : "0");
		}
	}
}
